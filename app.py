...
def update_profile():
    ...
    if form.validate_on_submit():
        if User.authenticate(user.username, form.password.data):
            ...
            flash("Invalid password. Profile was not updated.", "danger")
            return redirect("/")
    else:
...
def add_like(message_id):
    ...
    if liked_message.user == g.user:
        flash("You cannot like or unlike your own message.", "danger")
        return redirect(f"/users/{g.user.id}")
...
def show_likes(user_id):
    ...
    # commented out so i can access during development
    # if not g.user:
    #     flash("Access unauthorized.", "danger")
    #     return redirect("/")
    user = User.query.get_or_404(user_id)
    user_likes = user.likes 
    user_likes_ids = [like.id for like in user_likes]
    ...
    if g.user:
        ...
        messages = (Message
                    .query
                    .filter(Message.id.in_(user_likes_ids))
                    .order_by(Message.timestamp.desc())
                    .limit(100)
                    .all())
